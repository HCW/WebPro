<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="java.util.*,com.tool.adUser" errorPage="../error.jsp"%>
...
<div class="container clearfix">
    ...
    <div class="main-wrap">
        ...
        <div class="crumb-wrap">
            <div class="crumb-list"><i class="icon-font"></i><a href="index.jsp">首页</a><span class="crumb-step">&gt;</span><span class="crumb-name"><a href="adminmanager.jsp">管理员管理</a></span><span class="crumb-step">&gt;</span><span class="crumb-name">修改管理员</span></div>
        </div>
            ...
            <div class="result-content">
                <form action="" method="post" id="myform" name="myform" enctype="multipart/form-data">
                    <table class="insert-tab" width="100%">
						...
						Collection<adUser> temp = sql.selectadUserIN(id);
						Iterator<adUser> it = temp.iterator();

						session.setAttribute("id", id);
						
						while (it.hasNext()) {
							adUser user = (adUser) it.next();
						
					%>
                            <tr>
                                <th><i class="require-red">*</i>管理员ID：</th>
                                <td>
                                    <input class="common-text required" id="aid" name="title" size="50" value="<%=user.getID()%>" type="text" readonly="true">
                                </td>
                            </tr>
                            <tr>
                                <th><i class="require-red">*</i>用户名：</th>
                                <td>
                                    <input class="common-text required" id="name" name="author" size="50" value="<%=user.getName()%>" type="text">
                                </td>
                            </tr>
                            <tr>
                            ...
                            <td>
                                <select name="colId" id="per" class="required" >
                                    <option value="<%=user.getPer()%>"><%=user.getPer()%></option>
                            ...
                            <tr>
                                <th></th>
                                <td>
                                    <input class="btn btn-primary btn6 mr10" value="修改" type="button" onclick="UpdateAdmin()">
...
function UpdateAdmin(){
	...
	if(Aid != ""){
		//alert("修改成功");
		$.post("/NewEnterpriseProject/DoAdminUpdate", {"aid":Aid,"name":Name,"pwd":Pwd,"per":Per},function(rst){
			if(rst == "success"){
				alert("修改成功");
				window.location.href= "/NewEnterpriseProject/partII/UpdateAdmin.jsp?id="+Aid;
			}
			else{
				alert("修改失败");
			}
