<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="java.util.*,com.tool.Message" errorPage="../error.jsp"%>
...
<div class="container clearfix">
    ...
    <div class="main-wrap">
        ...
        <div class="crumb-wrap">
            <div class="crumb-list"><i class="icon-font"></i><a href="index.jsp">首页</a><span class="crumb-step">&gt;</span><span class="crumb-name">留言管理</span></div>
        </div>
            ...
            <form name="myform" id="myform" method="post">
                <div class="result-title">
                    <div class="result-list">
                        <a href="AddMessage.jsp"><i class="icon-font"></i>添加留言</a>
                ...
                <div class="result-content">
                    <table class="result-tab" width="100%">
                        ...
                        <% 
                        Collection<Message> temp=sql.selectMessage();
                        Iterator<Message> it=temp.iterator();
                        int Count=0;
                        while(it.hasNext())
                        {
                        	Message newsl=(Message)it.next();
                        	out.println("<tr>");
                        	out.println("<td class='tc'><input name='id[]' value='' type='checkbox'></td>");
                        	out.println("<td>"+newsl.getTID()+"</td>");
                        	out.println("<th>"+newsl.getName()+"</th>");
                        	out.println("<td>"+newsl.getDate()+"</td>");
                        	out.println("<td>");
                        	out.println("<a class='link-update' href='MessageUpdate.jsp?id="+newsl.getTID()+"'>修改</a>");
                        	out.println("<a class='link-update' onclick='DelMsg("+newsl.getTID()+")'>删除</a>");
                        	out.println("<a class='link-update' href='MessageView.jsp?id="+newsl.getTID()+"'>查看</a>");
                        	out.println("</td>");
                        	out.println("</tr>");
                        	Count++;
                        }
...
function DelMsg(ID)
{
	if (confirm("你确定要删除该留言吗？")) {  
		$.post("/NewEnterpriseProject/DoMessageDel", {"tid":ID},function(rst){
			if(rst == "success"){
				alert("删除成功！");
				window.location.href= "/NewEnterpriseProject/partII/messageManager.jsp";
			}
			else{
				alert("删除失败！");
			}
