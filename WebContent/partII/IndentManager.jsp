<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="java.util.*,com.tool.Indent" errorPage="../error.jsp"%>
...
<div class="container clearfix">
    ...
    <div class="main-wrap">
        ...
        <div class="crumb-wrap">
            <div class="crumb-list"><i class="icon-font"></i><a href="index.jsp">首页</a><span class="crumb-step">&gt;</span><span class="crumb-name">订单管理</span></div>
        </div>
            ...
            <form name="myform" id="myform" method="post">
                <div class="result-title">
                    <div class="result-list">
                        <a href="AddIndent.jsp"><i class="icon-font"></i>添加订单</a>
                ...
                <div class="result-content">
                    <table class="result-tab" width="100%">
                        ...
                        <% 
                        Collection<Indent> temp=sql.selectIndent();
                        Iterator<Indent> it=temp.iterator();
                        int Count=0;
                        while(it.hasNext())
                        {
                        	Indent newsl=(Indent)it.next();
                        	out.println("<tr>");
                        	out.println("<td class='tc'><input name='id[]' value='' type='checkbox'></td>");
                        	out.println("<td>"+newsl.getID()+"</td>");
                        	out.println("<th>"+newsl.getMName()+"</th>");
                        	out.println("<td>"+newsl.getPName()+"</td>");
                        	out.println("<td>");
                        	out.println("<a class='link-update' href='IndentUpdate.jsp?id="+newsl.getID()+"'>修改</a>");
                        	out.println("<a class='link-update' onclick='DelIndent("+newsl.getID()+")'>删除</a>");
                        	out.println("<a class='link-update' href='IndentView.jsp?id="+newsl.getID()+"'>查看</a>");
                        	out.println("</td>");
                        	out.println("</tr>");
                        	Count++;
                        }
...
<script type="text/javascript">
function DelIndent(ID)
{
	if (confirm("你确定要删除该留言吗？")) {  
		$.post("/NewEnterpriseProject/DoIndentDel", {"id":ID},function(rst){
			if(rst == "success"){
				alert("删除成功！");
				window.location.href= "/NewEnterpriseProject/partII/IndentManager.jsp";
			}
			else{
				alert("删除失败！");
			}
