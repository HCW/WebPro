            ...
            <form name="myform" id="myform" method="post">
                <div class="result-title">
                    <div class="result-list">
                        <a href="AddAdmin.jsp"><i class="icon-font"></i>添加管理员</a>
                ...
                <div class="result-content">
                    <table class="result-tab" width="100%">
                        ...
                        <% 
                        Collection<adUser> temp=sql.selectadUser();
                        Iterator<adUser> it=temp.iterator();
                        int Count=0;
                        while(it.hasNext())
                        {
                        	adUser newsl=(adUser)it.next();
                        	out.println("<tr>");
                        	out.println("<td class='tc'><input name='id[]' value='' type='checkbox'></td>");
                        	out.println("<td>"+newsl.getID()+"</td>");
                        	out.println("<th>"+newsl.getName()+"</th>");
                        	out.println("<td>"+newsl.getPer()+"</td>");
                        	out.println("<td>");
                        	out.println("<a class='link-update' href='UpdateAdmin.jsp?id="+newsl.getID()+"'>修改</a>");
                        	out.println("<a class='link-update' onclick='DelAdmin("+newsl.getID()+")'>删除</a>");
                        	out.println("<a class='link-update' href='AdminView.jsp?id="+newsl.getID()+"'>详情</a>");
                        	out.println("</td>");
                        	out.println("</tr>");
                        	Count++;
                        }
...
function DelAdmin(ID)
{
	if (confirm("你确定要删除该管理员吗？")) {  
		$.post("/NewEnterpriseProject/DoAdminDel",{"aid":ID},function(rst){
			if(rst == "success"){
				alert("删除成功！");
				window.location.href= "/NewEnterpriseProject/partII/adminmanager.jsp";
			}
			else{
				alert("删除失败！");
			}
